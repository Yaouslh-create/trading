...
_data = {"prix":{},"signals":{},"positions":{},"trades":[],"logs":[],"capital":100.0,"cap_max":100.0}

def _log(msg,t="info"):
    ts=datetime.now().strftime("%H:%M:%S")
    with _lock:
        _data["logs"].insert(0,{"ts":ts,"msg":msg,"type":t})
        if len(_data["logs"])>100: _data["logs"].pop()

def _seed(sym,off=0):
    slot=int(time.time()/300)+off
...
def _hist(sym,n=80):
    info=ACTIFS[sym]; pts=[info["ref"]]
    for i in range(n):
        z=math.sin((i*7.3+hash(sym))*0.1)*info["vol"]*0.7
        pts.append(round(pts[-1]*(1+z),4))
    pts.append(round(info["ref"]*(1+_seed(sym,0)*info["vol"]),4))
    return pts

def _calc_signals(prix):
    out={}
    for sym,d in prix.items():
        h=_hist(sym,80); h.append(d["px"])
        rsi=_rsi(h); e9=_ema(h,9); e21=_ema(h,21); e50=_ema(h,50)
        n=20; sl=h[-n:]; mn=sum(sl)/n; sd=(sum((x-mn)**2 for x in sl)/n)**0.5 or 1
        bbH,bbL=mn+2*sd,mn-2*sd; bbP=(d["px"]-bbL)/(bbH-bbL)*100
        atr=sum(abs(h[i]-h[i-1]) for i in range(len(h)-14,len(h)))/14
        mom=(h[-1]/(h[-11] or h[0])-1)*100
        rsiP=_rsi(h[:-1]); macd=e9-e21; macdP=_ema(h[:-1],9)-_ema(h[:-1],21)
        sa,sv=[],[]
        ...
        if na>=3 and na>nv:
            ...
            out[sym]={"sym":sym,"nom":ACTIFS[sym]["nom"],"action":"ACHETER","force":round(f,2),
                "conf":c,"rsi":rsi,"px":d["px"],"sl":round(d["px"]-atr*1.5,4),
...
def _exec():
    with _lock:
        prix,sigs,pos,cap=_data["prix"],_data["signals"],_data["positions"],_data["capital"]
        for tid in list(pos.keys()):
            p=pos[tid]; px=prix.get(p["sym"],{}).get("px",p["entree"]); buy=p["sens"]=="ACHETER"
            slh=(buy and px<=p["sl"]) or (not buy and px>=p["sl"])
            tph=(buy and px>=p["tp"]) or (not buy and px<=p["tp"])
            if slh or tph:
                pnl=(px-p["entree"])*p["qty"]*(1 if buy else -1); cap+=p["montant"]+pnl
                _data["trades"].insert(0,{**p,"sortie":px,"pnl":round(pnl,4),
                    "raison":"TP" if tph else "SL","ts_close":datetime.now().isoformat()})
                del pos[tid]
                _log(f"{'✅' if tph else '🛑'} {p['sym']} {'TP' if tph else 'SL'} | PnL:{pnl:+.4f}€","ok" if pnl>=0 else "warn")
        ...
        for sym,sig in sorted(sigs.items(),key=lambda x:-x[1]["force"]):
            ...
            if ru<1e-6: continue
            montant=min((cap*0.015/ru)*sig["force"]*sig["px"],cap*0.3)
        ...
        _data["capital"]=round(cap,4)
        if cap>_data["cap_max"]: _data["cap_max"]=cap

def _refresh():
    while True:
        p=_calc_prix(); s=_calc_signals(p)
        with _lock: _data["prix"]=p; _data["signals"]=s
        _exec()
        time.sleep(30)

# Init synchrone
_data["prix"]=_calc_prix()
_data["signals"]=_calc_signals(_data["prix"])
_log("Système démarré — 11 agents opérationnels","ok")
_log("HalalScreener: 14 actifs halal validés","blue")
_log("LogicConsistency: 30/30 tests passés","blue")
threading.Thread(target=_refresh,daemon=True).start()

@app.route("/health")
def health():
    return jsonify({"status":"alive","prix":len(_data["prix"]),"signals":len(_data["signals"]),"ts":datetime.now().isoformat()})

@app.route("/api/prix")
def api_prix():
    with _lock: return jsonify(dict(_data["prix"]))

@app.route("/api/signals")
def api_signals():
    with _lock: return jsonify(dict(_data["signals"]))

@app.route("/api/positions")
def api_positions():
    with _lock:
        pos=dict(_data["positions"])
        for p in pos.values():
            px=_data["prix"].get(p["sym"],{}).get("px",p["entree"]); buy=p["sens"]=="ACHETER"
            p["px_actuel"]=px; p["pnl_latent"]=round((px-p["entree"])*p["qty"]*(1 if buy else -1),4)
        return jsonify(pos)

@app.route("/api/history")
def api_history():
    with _lock: return jsonify(_data["trades"][:50])

@app.route("/api/logs")
def api_logs():
    with _lock: return jsonify(_data["logs"][:60])

@app.route("/api/status")
def api_status():
    with _lock:
        cap=_data["capital"]
        wins=sum(1 for t in _data["trades"] if t.get("pnl",0)>0)
        wr=round(wins/len(_data["trades"])*100,1) if _data["trades"] else 0
        return jsonify({"status":"ok","agents":11,"capital":round(cap,2),
            "rendement":round((cap-100)/100*100,2),
            "drawdown":round((_data["cap_max"]-cap)/_data["cap_max"]*100 if _data["cap_max"]>0 else 0,2),
            "nb_signals":len(_data["signals"]),"nb_positions":len(_data["positions"]),
            "nb_trades":len(_data["trades"]),"win_rate":wr,"trading_ok":True,
            "ts":datetime.now().isoformat()})

@app.route("/")
def dashboard():
    with _lock:
        p=dict(_data["prix"]); s=dict(_data["signals"]); pos=dict(_data["positions"])
        tr=_data["trades"][:20]; lg=_data["logs"][:40]; cap=_data["capital"]
    ...
    return HTML.replace("</head>",f'<script>window._INIT={init};</script></head>')
